...
public class EditorTools
{
	...
	public static int DrawTagGUI (int selectID,string[] tagNames,ref int startID,int showTagCount,params GUILayoutOption[] options)
	{
		...
		for (int i = 0; i < showTagNames.Length; i++)
		{
			...
			if(allShow)
			{
				if(showTagNames.Length > 1)
				{
					if(i == 0)
						thisButtonStyle = EditorStyles.miniButtonLeft;
					else if(i == showTagNames.Length - 1)
	...
	public static void SpaceLine(float size)
	{
		...
		GUILayout.Space(2);
		GUI.enabled = false;
		GUILayout.TextField("", GUILayout.Height(size));
		GUI.enabled = true;
		GUILayout.Space(5);
	...
	public static void TitleLine (string Title,float lineSize)
	{
		...
		GUI.enabled = false;
		GUILayout.TextField("", GUILayout.Height(lineSize));
		GUI.enabled = true;
		GUILayout.Label (Title,GUILayout.Width (Title.Length*10));
		GUILayout.Space(-5);
		GUILayout.EndVertical ();
	}
